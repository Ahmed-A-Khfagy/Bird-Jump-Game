-- Author : Ahmed A. Khfagy
-- made in CS50 into to Game Development

-- this class represents the Playing state

PlayState = Class{__includes = BaseState}

PIPE_SPEED = 60
PIPE_WIDTH = 70
PIPE_HEIGHT = 288

BIRD_WIDTH = 38
BIRD_HEIGHT = 24

math.randomseed(os.time())

tToSpawnPipesUpon = math.random(2, 5)

pause = false

function PlayState:init()

    self.bird = Bird()

    self.pipePairs = {}

    self.timer = 0

    self.score = 0

    self.lastY = -PIPE_HEIGHT + math.random(80) + 20

end


function PlayState:update(dt)

    if not pause then

        self.timer = self.timer + dt

        if self.timer > tToSpawnPipesUpon then

            local y = math.max(-PIPE_HEIGHT + 10, math.min(self.lastY + math.random(-20, 20), VIRTUAL_HEIGHT - 90 - PIPE_HEIGHT))

            self.lastY = y

            table.insert(self.pipePairs, PipePair(y))

            self.timer = 0

        end

        for i, pair in pairs(self.pipePairs) do

            if not pair.scored then

                if pair.x + PIPE_WIDTH <= self.bird.x then

                    self.score = self.score + 1

                    pair.scored = true

                    sounds['score']:play()

                end

            end

            pair:update(dt)

        end

        for i, pair in pairs(self.pipePairs) do 

            if pair.remove then

                table.remove(self.pipePairs, i)

            end

        end

        self.bird:update(dt)

        for i, pair in pairs(self.pipePairs) do 

            for j, pipe in pairs(pair.pipes) do

                if self.bird:collides(pipe) then

                    sounds['explosion']:play()

                    sounds['hurt']:play()

                    gStateMachine:change('score', self.score)

                end

            end

        end

        if self.bird.y > VIRTUAL_HEIGHT - 15 then
            
            sounds['explosion']:play()

            sounds['hurt']:play()

            gStateMachine:change('score', self.score)

        end

    end

    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then

        pause = not pause

        scrolling = not scrolling

    end

end

function PlayState:render()

    for i, pair in pairs(self.pipePairs) do

        pair:render()

    end

    love.graphics.setFont(flappyFont)
    love.graphics.print('Score: ' .. tostring(self.score), 8, 8)

    self.bird:render()

    if pause then 

        love.graphics.setFont(hugeFont)
        love.graphics.printf('PAUSED!!', 0, 100, VIRTUAL_WIDTH, 'center')

        love.graphics.setFont(mediumFont)
        love.graphics.printf('Press enter to continue..!', 0, 160, VIRTUAL_WIDTH, 'center')

    else

        love.graphics.setFont(smallFont)
        love.graphics.print('Press enter to pause..!', 8, 50)

    end

end

function PlayState:enter()

    scrolling = true

    tToSpawnPipesUpon = math.random(2, 5)
    
    gapHeight = math.random(80, 120)
    
end 

function PlayState:exit()

    scrolling = false

end
